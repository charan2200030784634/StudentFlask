    ...
    if request.method == "POST":
        try:
            ...
            with sqlite3.connect("student_detials.db") as connection:
                ...
                cursor.execute(
                    "INSERT into Student_Info (name, email, gender, contact, dob, address) values (?,?,?,?,?,?)",
...
def updaterecord():
    ...
    with sqlite3.connect("student_detials.db") as connection:
        ...
        if cursor.fetchone() is not None:
            cursor.execute("update Student_Info set name=?, email=?, gender=?, contact=?, dob=?, address=? where id=?",
                           (name, email, gender, contact, dob, address, id))
            connection.commit()
            msg = "Student record updated successfully!"
        else:
            msg = "Record not found. Update failed."
